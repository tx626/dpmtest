...
def test():
    ...
    for i in range(TEST_ROUNDS):
        ...
        if not counter:
            log_err('%%%%%%%% get_packages_details->get_counter', 'failed to get the total number of %s ' % str(CATEGORY))
            return
        if SHOW_TIME:
            log_debug('get_packages_details->get_counter', 'time=%d sec' % (datetime.utcnow() - start_time).seconds)
        ...
        ret = json.loads(ret)
        if not ret:
            log_err('%%%%%%%% get_packages_details', 'failed to get package details')
            return
        else:
